...
SOIL_DEPTHS = np.array([-1.945, -0.64, -0.175, -0.035], dtype=np.float64)
ROOT_FRACTION = np.array(
    [0.0385487373669315, 0.307407403941806, 0.409283067913477, 0.244760790777786],
    dtype=np.float64,
)
...
def open_era5_dataset(files: list[Path]) -> xr.Dataset:
    ...
    for path in files:
        if first_suffix in (".grib", ".grb"):
            ...
            if "_surface_" in path.stem:
                # Keep the ordinary single-level fields in the base group;
                # soil layers are merged below from their depth groups.
            ...
            # Soil layers are separate GRIB messages with different
            # depthBelowLandLayer coordinates. cfgrib cannot combine those
            # messages in one dataset and otherwise skips layers 2--4.
            soil_datasets = []
            for level in (0, 7, 28, 100):
                soil_ds = xr.open_dataset(
                    path,
                    engine="cfgrib",
                    backend_kwargs={
                        "indexpath": "",
                ...
                if soil_ds.data_vars:
                    soil_datasets.append(
                        soil_ds.drop_vars("depthBelowLandLayer", errors="ignore")
                    )
            if soil_datasets:
                ds = ds.drop_vars("depthBelowLandLayer", errors="ignore")
                ds = xr.merge([ds, *soil_datasets], compat="override", join="exact")
        else:
...
def center_soil_type(field: np.ndarray, lon: np.ndarray, lat: np.ndarray, cfg: dict) -> int:
    """Select a categorical ERA5 soil type without averaging sea points."""
    values = np.asarray(field[0], dtype=np.float64)
    n_av = int(cfg["era5"].get("n_av", 3))
    i = int(np.abs(lon - float(cfg["era5"]["central_lon"])).argmin())
    j = int(np.abs(lat - float(cfg["era5"]["central_lat"])).argmin())

    def valid_category(value: float) -> bool:
        return np.isfinite(value) and value >= 1.0 and np.isclose(value, np.rint(value))

    if valid_category(values[j, i]):
        return int(np.rint(values[j, i]))

    j0 = max(0, j - n_av)
    j1 = min(lat.size, j + n_av + 1)
    i0 = max(0, i - n_av)
    i1 = min(lon.size, i + n_av + 1)
    window = values[j0:j1, i0:i1]
    valid = np.isfinite(window) & (window >= 1.0) & np.isclose(window, np.rint(window))
    if not valid.any():
        raise ValueError(
            "Could not find a valid ERA5 soil type at the domain center or "
            f"within its {2 * n_av + 1}x{2 * n_av + 1} averaging window."
        )

    rows, columns = np.nonzero(valid)
    distance_sq = (rows + j0 - j) ** 2 + (columns + i0 - i) ** 2
    nearest = int(np.argmin(distance_sq))
    return int(np.rint(window[rows[nearest], columns[nearest]]))


def center_soil_average(
        field: np.ndarray,
        soil_type: np.ndarray,
        selected_soil_type: int,
        lon: np.ndarray,
        lat: np.ndarray,
        cfg: dict,
        label: str) -> float:
    """Average an initial soil field over nearby points of one soil class."""
    n_av = int(cfg["era5"].get("n_av", 3))
    i = int(np.abs(lon - float(cfg["era5"]["central_lon"])).argmin())
    j = int(np.abs(lat - float(cfg["era5"]["central_lat"])).argmin())
    j0 = max(0, j - n_av)
    ...
    values = np.asarray(field[0, j0:j1, i0:i1], dtype=np.float64)
    categories = np.asarray(soil_type[0, j0:j1, i0:i1], dtype=np.float64)
    valid = np.isfinite(values) & np.isclose(categories, selected_soil_type)
    if not valid.any():
        raise ValueError(
            f"Could not find finite {label} values for ERA5 soil type "
            f"{selected_soil_type} in the center averaging window."
        )
...
def read_era5(cfg: dict, include_radiation: bool) -> Era5Data:
    ...
    # ERA5 stores soil layers from the surface downward. MicroHH's soil
    # index 0 is the deepest layer, matching the ordering used by Cabauw.
    soil_type_da = data_var(ds_s, ("slt", "soil_type"), required=False)
    if soil_type_da is None:
        raise KeyError("LSM input requires ERA5 soil_type (slt).")
    soil_type = to_3d(soil_type_da, "time", surf_lat_name, surf_lon_name)
    selected_soil_type = center_soil_type(
        soil_type,
        ds_s[surf_lon_name].values,
        ds_s[surf_lat_name].values,
        cfg,
    )
    ...
    for index in range(1, 5):
        temperature_da = data_var(
            ds_s,
            (f"stl{index}", f"soil_temperature_level_{index}"),
            required=False,
        )
        moisture_da = data_var(
            ds_s,
            (f"swvl{index}", f"volumetric_soil_water_layer_{index}"),
            required=False,
        )
        if temperature_da is None or moisture_da is None:
            raise KeyError(
                "LSM input requires ERA5 soil_temperature_level_1..4 and "
                "volumetric_soil_water_layer_1..4."
        )
        temperature = to_3d(temperature_da, "time", surf_lat_name, surf_lon_name)
        moisture = to_3d(moisture_da, "time", surf_lat_name, surf_lon_name)
        soil_temperature.append(center_soil_average(
            temperature,
            soil_type,
            selected_soil_type,
            ds_s[surf_lon_name].values,
            ds_s[surf_lat_name].values,
            cfg,
            f"soil temperature level {index}",
        ))
        soil_moisture.append(center_soil_average(
            moisture,
            soil_type,
            selected_soil_type,
            ds_s[surf_lon_name].values,
            ds_s[surf_lat_name].values,
            cfg,
            f"soil moisture layer {index}",
        ))

    soil_index = selected_soil_type - 1

    soil = {
        "z": SOIL_DEPTHS.copy(),
        "theta_soil": np.asarray(soil_moisture[::-1], dtype=np.float64),
        "t_soil": np.asarray(soil_temperature[::-1], dtype=np.float64),
        "index_soil": np.full(SOIL_DEPTHS.size, soil_index, dtype=np.int32),
        "root_frac": ROOT_FRACTION.copy(),
    }
...
def add_nc_var(name: str, dims, nc_group, data, float_type, dtype=None):
    if name in nc_group.variables:
        return
    if dims is None:
        var = nc_group.createVariable(name, dtype or float_type)
    else:
        var = nc_group.createVariable(name, dtype or float_type, dims)
    var[:] = data
...
def write_case_input(cfg: dict, exp_dir: Path, grid: GridData, era5: Era5Data, float_type, run_cfg: dict | None = None):
    ...
    nc_soil = nc_main.createGroup("soil")
    add_nc_dim("z", era5.soil["z"].size, nc_soil)
    add_nc_var("z", ("z",), nc_soil, era5.soil["z"], float_type)
    add_nc_var("theta_soil", ("z",), nc_soil, era5.soil["theta_soil"], float_type)
    add_nc_var("t_soil", ("z",), nc_soil, era5.soil["t_soil"], float_type)
    add_nc_var(
        "index_soil", ("z",), nc_soil, era5.soil["index_soil"], float_type, dtype=np.int32
    )
    add_nc_var("root_frac", ("z",), nc_soil, era5.soil["root_frac"], float_type)
...
def surface_restart_fields(cfg: dict) -> list[str]:
    base_ini = resolve_path(
        cfg["case"].get("base_ini", "era5_openbc.ini.base"),
        cfg["_base_dir"],
    )
    boundary = io.read_ini(str(base_ini))["boundary"]
    common = ["thl_bot", "qt_bot", "dudz_mo", "dvdz_mo", "dbdz_mo"]

    if str(boundary.get("swboundary", "")).lower() != "surface_lsm":
        return [*common, "obuk"]

    fields = [*common, "t_soil", "theta_soil", "wl_skin"]
    for tile in ("veg", "soil", "wet"):
        fields.extend((f"thl_bot_{tile}", f"qt_bot_{tile}"))

    if not bool(boundary.get("swconstantz0", False)):
        fields.extend(f"obuk_{tile}" for tile in ("veg", "soil", "wet"))
    return fields
...
def link_outer_run_inputs(cfg: dict, exp_dir: Path):
    ...
    restart_fields.update(surface_restart_fields(cfg))
    restart_fields.update({"time", "thermo_basestate"})
...
def copy_lsm_lookup(cfg: dict, exp_dir: Path):
    """Install the van Genuchten lookup table required by surface_lsm."""
    base_cfg = cfg.get("radiation", {})
    lsm_cfg = cfg.get("land_surface", {})
    microhh_root = resolve_path(
        lsm_cfg.get("microhh_root", base_cfg.get("microhh_root", "../..")),
        cfg["_base_dir"],
    )
    src = microhh_root / "misc" / "van_genuchten_parameters.nc"
    if not src.exists():
        raise FileNotFoundError(f"Missing land-surface lookup table: {src}")

    dst = exp_dir / "van_genuchten_parameters.nc"
    if dst.exists() or dst.is_symlink():
        dst.unlink()
    if bool(lsm_cfg.get("link_lookup", True)):
        dst.symlink_to(src.resolve())
    else:
        shutil.copy(src, dst)
