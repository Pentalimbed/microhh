...
namespace
{
    ...
    void run_model_visualizer(Model_viz<TF>& model)
    {
        model.prepare_run();

        try
        {
            run_visualizer(model);
        }
        catch (...)
        {
            model.finalize_run();
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (master.get_MPI_data().nprocs != 1)
            throw std::runtime_error("microhh_viz currently supports serial runs only");
        ...
        Model_viz<float> model(master, argc, argv);
        #else
        master.print_message("Precision: Double (64-bits floats)\n");
        Model_viz<double> model(master, argc, argv);
        #endif

        // Initialize the model components.
        model.init();

        // Load the restart data created by the normal MicroHH init stage.
        model.load_or_save();

        // Run the GUI-controlled visualizer.
        run_model_visualizer(model);
    }
